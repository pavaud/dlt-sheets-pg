...
def google_spreadsheet(
    ...
    assert len(all_range_names) == len(all_range_data), "Google Sheets API must return values for all requested ranges"
    ...
    for name, parsed_range, meta_range, values in all_range_data:
        ...
        if len(values) == 1:
            ...
            continue
        if len(values[0]) == 0:
            logger.warning(f"First row of range {name} does not contain data. Skipping.")
            continue
        metadata_table[-1]["skipped"] = False
        range_data.append((name, parsed_range, meta_range, values))

    meta_values = api_calls.get_meta_for_ranges(service, spreadsheet_id, [str(data[2]) for data in range_data])
    for name, parsed_range, _, values in range_data:
        ...
        metadata = next(
            sheet for sheet in meta_values["sheets"] if sheet["properties"]["title"] == parsed_range.sheet_name
        )["data"].pop(0)
        ...
        if headers is None:
            ...
            rows_data = values[0:]
            logger.warning(f"Using automatic headers. WARNING: first row of the range {name} will be used as data!")
        else:
